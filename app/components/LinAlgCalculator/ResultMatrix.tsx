...
export default function ResultMatrix({ label, matrix }: ResultMatrixProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      {label && <h3 className="text-sm text-[color:var(--ss-text-soft)] mb-2">{label}</h3>}
        ...
        >
          {matrix.map((row, r) =>
            row.map((value, c) => (
              <span
                key={`${r}-${c}`}
                className="min-w-10 text-center text-sm text-[color:var(--ss-text)] tabular-nums"
              >
                {formatNumber(value)}
              </span>
            ))
...
function Bracket({ side }: { side: "left" | "right" }) {
  return (
    <div
      className={`w-2 border-y-2 border-[color:var(--ss-text-soft)] ${
